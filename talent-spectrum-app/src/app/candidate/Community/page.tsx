...
import {
  Users, MessageSquare, Rss, Search, User, Heart, MessageCircle, Plus, Settings, Hash,
  MapPin, Briefcase, Book, Star, Smile, Zap, Lightbulb, Sun, Moon, Coffee, Image as ImageIcon,
  Globe, Lock, Mail, Phone, Calendar, Tag, Info, UserCheck, XCircle
} from 'lucide-react';
...
interface BadgeProps extends React.HTMLAttributes<HTMLDivElement> {
  variant?: 'default' | 'success' | 'warning' | 'danger' | 'info' | 'purple' | 'blue' | 'green' | 'yellow' | 'red';
  children: React.ReactNode;
  className?: string;
}
...
const NeuroConnectPage: React.FC = () => {
  ...
  const suggestedUsers: UserProfile[] = [
    { id: 'u1', name: 'Muhammad Aiman', avatar: '/community/muhammad_aiman.png', neurotype: 'ADHD', interests: ['Writing', 'Photography', 'Travel'], mutual: 3 },
    { id: 'u2', name: 'Arjun Raj', avatar: '/community/arjun_raj.png', neurotype: 'Autistic', interests: ['Coding', 'Sci-Fi', 'Chess'], mutual: 5 },
    { id: 'u3', name: 'Nur Aisyah', avatar: '/community/nur_aisyah.png', neurotype: 'Dyslexic', interests: ['Reading', 'History', 'Cooking'], mutual: 1 },
    { id: 'u4', name: 'Lim Jun Hao', avatar: '/community/lim_jun_hao.png', neurotype: 'Autistic', interests: ['Art', 'Music', 'Animals'], mutual: 2 },
    { id: 'u5', name: 'Siti Nurina', avatar: '/community/siti_nurina.png', neurotype: 'Dyslexic', interests: ['Reading', 'History', 'Cooking'], mutual: 1 },
    { id: 'u6', name: 'Emma Webber', avatar: '/community/emma_webber.png', neurotype: 'Autistic', interests: ['Art', 'Music', 'Animals'], mutual: 2 },
  ];
  ...
  const [communityPosts, setCommunityPosts] = useState<CommunityPost[]>([
    {
      ...
      content: "Just finished a new short story! It's about a neurodivergent protagonist discovering their unique strengths. Anyone else find writing helps process thoughts?",
    ...
    },
    {
      ...
      content: "Looking for recommendations for sensory-friendly headphones for work. Any suggestions that are comfortable for long periods?",
    ...
    },
    {
      ...
      content: "Feeling a bit overwhelmed today. Just wanted to share that it's okay to take a break and recharge. Sending good vibes to everyone!",
      timestamp: '1 day ago',
      likes: 40,
      comments: 12,
  ...
  const communityGroups: CommunityGroup[] = [
    { id: 'g1', name: 'Autism & Tech', description: 'Discussing tech, coding, and innovation from an autistic perspective.', members: 120, imageUrl: '/community/group/autism_tech.png', isJoined: true },
    { id: 'g2', name: 'ADHD Productivity Hacks', description: 'Sharing strategies and tools for focus and organization with ADHD.', members: 85, imageUrl: '/community/group/adhd_pd.png', isJoined: false },
    { id: 'g3', name: 'Neurodivergent Artists', description: 'A space for neurodivergent creators to share art and inspiration.', members: 200, imageUrl: '/community/group/neurodivergence_artist.png', isJoined: true },
    { id: 'g4', name: 'Sensory-Friendly Living', description: 'Tips and tricks for creating a comfortable and sensory-aware environment.', members: 150, imageUrl: '/community/group/sensory_living.png', isJoined: false },
  ];

  const messages: Conversation[] = [
    { id: 'm1', sender: 'Ben Carter', senderAvatar: '/community/ben_carter.png', lastMessage: 'Hey Alex, saw your post about gaming. What are you playing lately?', time: '10:30 AM', chatHistory: [
        { type: 'received', text: 'Hey Alex, saw your post about gaming. What are you playing lately?' },
        { type: 'sent', text: 'Hey Ben! Mostly indie games and some strategy titles. Just finished Stardew Valley again!' },
        { type: 'received', text: 'Oh, Stardew is great! I\'ve been looking for something new. Any other recommendations?' },
      ]
    },
    { id: 'm2', sender: 'Maya Singh', senderAvatar: '/community/maya_singh.png', lastMessage: 'Thanks for the feedback on my story! Really appreciate it.', time: 'Yesterday', chatHistory: [
        { type: 'received', text: 'Thanks for the feedback on my story! Really appreciate it.' },
        { type: 'sent', text: 'You\'re welcome! Your writing is truly inspiring.' },
      ]
    },
    { id: 'm3', sender: 'Chloe Lee', senderAvatar: '/community/chloe_lee.png', lastMessage: 'Feeling a bit overwhelmed today. Just wanted to share that it\'s okay to take a break and recharge.', time: '2 days ago', chatHistory: [
        { type: 'received', text: 'Feeling a bit overwhelmed today. Just wanted to share that it\'s okay to take a break and recharge.' },
        { type: 'sent', text: 'Sending you strength, Chloe! Take care of yourself.' },
      ]
    },
  ];

  const myConnections: UserProfile[] = [
    { id: 'c1', name: 'Ben Carter', avatar: '/community/ben_carter.png', neurotype: 'Autistic', interests: ['Coding', 'Sci-Fi', 'Chess'], lastActive: 'Online' },
    { id: 'c2', name: 'Maya Singh', avatar: '/community/maya_singh.png', neurotype: 'ADHD', interests: ['Writing', 'Photography', 'Travel'], lastActive: 'Active 30m ago' },
    { id: 'c3', name: 'Chloe Lee', avatar: '/community/chloe_lee.png', neurotype: 'Autistic', interests: ['Art', 'Music', 'Animals'], lastActive: 'Active 1h ago' },
    { id: 'c4', name: 'Siti Sarah', avatar: '/community/siti_sarah.png', neurotype: 'Dyspraxic', interests: ['Design', 'Gardening', 'Cooking'], lastActive: 'Offline' },
    { id: 'c5', name: 'Ahmad Firdaus', avatar: '/community/ahmad_firdaus.png', neurotype: 'Autistic', interests: ['Art', 'Music', 'Animals'], lastActive: 'Active 5h ago' },
    { id: 'c6', name: 'Kavitha', avatar: '/community/kavitha.png', neurotype: 'Dyspraxic', interests: ['Design', 'Gardening', 'Cooking'], lastActive: 'Offline' },
  ];
  ...
  const handleToggleCommentBox = (postId: string) => {
    setCommunityPosts(prevPosts =>
      prevPosts.map(post =>
        post.id === postId
          ? { ...post, showCommentBox: !post.showCommentBox, newCommentText: '' } // Clear text when toggling
          : post
      )
    );
  };
  ...
  const handleSubmitComment = (postId: string) => {
    setCommunityPosts(prevPosts =>
      prevPosts.map(post =>
        post.id === postId
          ? { ...post, comments: post.comments + 1, newCommentText: '', showCommentBox: false } // Simulate adding comment
          : post
      )
    );
  };
  ...
  const handleCreatePost = () => {
    ...
    const newPost: CommunityPost = { // Explicitly type newPost
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'feed':
        return (
          <div className="space-y-6">
            <h1 className="text-2xl font-bold text-[#3a4043]">Community Feed</h1>
            <Card className="p-4">
              <div className="flex items-start space-x-3 my-5">
                <img src={currentUser.avatar || '/avatar-placeholder.jpg'} alt={currentUser.name} className="w-10 h-10 rounded-full object-cover" />
                <Textarea
              ...
              {newPostImagePreview && (
                <div className="relative w-32 h-32 rounded-lg overflow-hidden mx-auto mb-4">
                  <img src={newPostImagePreview} alt="Image preview" className="w-full h-full object-cover" />
                  <Button
                    variant="ghost"
                    size="icon"
              ...
              <div className="flex justify-between items-center mt-4">
                ...
                <Button size="sm" onClick={handleCreatePost} disabled={!newPostText.trim() && !newPostImageFile}>
                  Post
                </Button>
              </div>
            </Card>
              ...
              >
                <Card className="p-6">
                  <div className="flex items-center space-x-3 mb-4">
                    <img src={post.authorAvatar || '/avatar-placeholder.jpg'} alt={post.author} className="w-12 h-12 rounded-full object-cover" />
                    <div>
                      <p className="font-semibold text-[#3a4043]">{post.author}</p>
                  ...
                  <div className="flex items-center space-x-4 text-gray-600 border-t border-[#e8e6f0] pt-4 mt-4">
                    <Button variant="ghost" size="sm" className="flex items-center space-x-1" onClick={() => handleLike(post.id)}>
                      <Heart className={`h-4 w-4 ${post.isLiked ? 'text-red-500 fill-red-500' : ''}`} /> <span className={post.isLiked ? 'font-bold' : ''}>{post.likes} Likes</span>
                    </Button>
                    <Button variant="ghost" size="sm" className="flex items-center space-x-1" onClick={() => handleToggleCommentBox(post.id)}>
                  ...
                  {post.showCommentBox && (
                    <div className="mt-4 pt-4 border-t border-[#e8e6f0] flex flex-col space-y-2">
                      ...
                      <div className="flex justify-end space-x-2">
                        <Button variant="outline" size="sm" onClick={() => handleToggleCommentBox(post.id)}>Cancel</Button>
                        <Button size="sm" onClick={() => handleSubmitComment(post.id)} disabled={!post.newCommentText.trim()}>Submit</Button>
                      </div>
                    </div>
                  )}
                </Card>
      ...
      case 'discover':
        return (
          <div className="space-y-6">
            <h1 className="text-2xl font-bold text-[#3a4043]">Discover Friends</h1>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {suggestedUsers.filter(user =>
                ...
                user.neurotype.toLowerCase().includes(searchQuery.toLowerCase()) ||
                ...
                >
                  <Card className="p-6 text-center">
                    <img src={user.avatar || '/avatar-placeholder.jpg'} alt={user.name} className="w-24 h-24 rounded-full object-cover mx-auto mb-4" />
                    <h3 className="text-xl font-semibold text-[#3a4043]">{user.name}</h3>
                    <p className="text-sm text-gray-600 mb-2">{user.neurotype}</p>
                    <div className="flex flex-wrap justify-center gap-2 mb-4">
                      {user.interests.map((interest, idx) => (
                        <Badge key={idx} variant="blue">{interest}</Badge>
                      ))}
                    </div>
                    <p className="text-xs text-gray-500 mb-4">{user.mutual} mutual connections</p>
            ...
            {suggestedUsers.filter(user =>
                ...
                <p className="text-center text-gray-600 text-lg mt-8">No users found matching your search criteria.</p>
              )}
          </div>
        );
      ...
      case 'connections':
        return (
          <div className="space-y-6">
            <h1 className="text-2xl font-bold text-[#3a4043]">My Connections</h1>
              ...
              {myConnections.filter(user =>
                ...
                >
                  <Card className="p-6 text-center">
                    ...
                    <p className="text-xs text-gray-500 mb-4">{user.lastActive}</p>
            ...
            {myConnections.filter(user =>
                ...
                <p className="text-center text-gray-600 text-lg mt-8">No connections found matching your search criteria.</p>
              )}
          </div>
        );
      ...
      case 'groups':
        return (
          <div className="space-y-6">
            <h1 className="text-2xl font-bold text-[#3a4043]">My Groups & Forums</h1>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {communityGroups.filter(group =>
                ...
                >
                  <Card className="p-6 text-center">
                    <img src={group.imageUrl || '/group-placeholder.jpg'} alt={group.name} className="w-24 h-24 rounded-full object-cover mx-auto mb-4" />
                    <h3 className="text-xl font-semibold text-[#3a4043]">{group.name}</h3>
                    ...
                    <Button className="w-full" variant={group.isJoined ? 'outline' : 'primary'}>
                      {group.isJoined ? (<><UserCheck className="h-4 w-4 mr-2" /> Joined</>) : (<><Plus className="h-4 w-4 mr-2" /> Join Group</>)}
                    </Button>
                  </Card>
                </motion.div>
              ))}
            ...
            {communityGroups.filter(group =>
                ...
                <p className="text-center text-gray-600 text-lg mt-8">No groups found matching your search criteria.</p>
              )}
          </div>
        );
        ...
        return (
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input id="name" value={currentUser.name} onChange={(e) => handleProfileChange('name', e.target.value)} />
                </div>
                <div>
                  <label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-1">Email Address</label>
                  <Input id="email" type="email" value={currentUser.email} onChange={(e) => handleProfileChange('email', e.target.value)} />
                </div>
                <div>
                  <label htmlFor="dob" className="block text-sm font-medium text-gray-700 mb-1">Date of Birth</label>
                  <Input id="dob" type="date" value={currentUser.dob} onChange={(e) => handleProfileChange('dob', e.target.value)} />
                </div>
                <div>
                  <label htmlFor="location" className="block text-sm font-medium text-gray-700 mb-1">Location</label>
                  <Input id="location" value={currentUser.location} onChange={(e) => handleProfileChange('location', e.target.value)} />
                </div>
                <div>
                  <label htmlFor="bio" className="block text-sm font-medium text-gray-700 mb-1">Bio</label>
                  <Textarea id="bio" value={currentUser.bio} onChange={(e) => handleProfileChange('bio', e.target.value)} />
                </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <label htmlFor="neurotype" className="block text-sm font-medium text-gray-700 mb-1">Neurotype</label>
                  <Select
                    onValueChange={(value) => handleProfileChange('neurotype', value)}
                ...
                <div>
                  ...
                  <Input
                    id="interests"
                    value={Array.isArray(currentUser.interests) ? currentUser.interests.join(', ') : currentUser.interests} // Ensure value is string
                    onChange={(e) => handleProfileChange('interests', e.target.value.split(',').map(s => s.trim()))}
                  />
                </div>
                <div>
                  <label htmlFor="privacy" className="block text-sm font-medium text-gray-700 mb-1">Profile Privacy</label>
                  <Select
                    onValueChange={(value) => handleProfileChange('privacy', value)}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Button onClick={() => alert('Password updated!')}>Change Password</Button>
              </CardContent>
            </Card>
          </div>
        );
    ...
    <div className="min-h-screen bg-gradient-to-b from-violet-50 to-background py-8 px-4 font-['Plus_Jakarta_Sans',_sans-serif]">
      <div className="max-w-[1400px] mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-[#3a4043]">NeuroConnect Community</h1>
            <p className="text-gray-600 mt-1">Connect, chat, and support each other in a safe space.</p>
          </div>
          <Button>
          ...
          <Card className="lg:col-span-1 lg:sticky lg:top-8 lg:max-h-[calc(100vh-4rem)] lg:overflow-y-auto">
            <CardContent className="p-6">
              <div className="flex items-center space-x-3 my-6">
                ...
                <div>
                  <p className="font-semibold text-[#3a4043]">{currentUser.name}</p>
                  <p className="text-sm text-gray-600 flex items-center"><User className="h-3 w-3 mr-1" /> {currentUser.neurotype}</p>
                </div>
              </div>
